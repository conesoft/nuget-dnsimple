...
public class Zone
{
    ...
    public async Task<IEnumerable<Record>> GetRecords() => (await client.GetJsonAsync<Records.Response?>($"{zone.account_id}/zones/{zone.id}/records"))?.data.Select(record => new Record(client, zone, record)) ?? Array.Empty<Record>();
    ...
    public async Task<Record?> AddRecord(RecordType type, string name, string content, TimeSpan timeToLife)
    {
        ...
        var record = (await response.Response<Records.SingleResponse?>())?.data ?? null;
        return record != null ? new Record(client, zone, record) : null;
    }
}